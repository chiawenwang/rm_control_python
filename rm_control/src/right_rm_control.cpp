...
void getArmError_Callback(const std_msgs::UInt16 msg)
{  
...
void execute(const control_msgs::FollowJointTrajectoryGoalConstPtr& goal, Server* as) {
    ...
    point_changed = false;
    std_msgs::Empty system_error;
    std_msgs::Bool timerSwitch;
    if(rm_arm_error == 4109)
    {
        system_error_clean_pub.publish(system_error);
    }
...
void timer_callback(const ros::TimerEvent)
{
    ...
    if(point_changed)
    {
        if(p2.vector_cnt < p2.vector_len)
        {
            ...
            joint_pos_msg.joint[0] = p_joint1_.at(p2.vector_cnt);
            joint_pos_msg.joint[1] = p_joint2_.at(p2.vector_cnt);
            joint_pos_msg.joint[2] = p_joint3_.at(p2.vector_cnt);
            joint_pos_msg.joint[3] = p_joint4_.at(p2.vector_cnt);
            joint_pos_msg.joint[4] = p_joint5_.at(p2.vector_cnt);
            joint_pos_msg.joint[5] = p_joint6_.at(p2.vector_cnt);
            if(arm_dof == 7)
            {
                joint_pos_msg.joint[6] = p_joint7_.at(p2.vector_cnt);
            }
        ...
        else
        {
            if(count_final_joint <= count_keep_send)
            {
                ...
                if(arm_dof == 7)
                {
                    joint_pos_msg.joint[6] = p_joint7_.at(p2.vector_cnt-1);
                }
                target_pub.publish(joint_pos_msg);
...
{
    ros::init(argc, argv, "right_rm_arm_control");
    ros::NodeHandle nh;
    ros::NodeHandle private_nh_("~");

    private_nh_.param<int>("Arm_Dof", arm_dof, 7);

    if(arm_dof == 6)
    {
        joint_pos_msg.joint.resize(6);
        ROS_INFO("arm_dof6 = %d",arm_dof);
    }
    else if(arm_dof == 7)
    {
        joint_pos_msg.joint.resize(7);
        ROS_INFO("arm_dof7 = %d",arm_dof);
    }
    ...
    get_ArmError = nh.subscribe("/right_rm_driver/ArmError", 100, getArmError_Callback);
    system_error_clean_pub = nh.advertise<std_msgs::Empty>("/right_rm_driver/Clear_System_Err", 300);
    target_pub = nh.advertise<rm_msgs::JointPos>("/right_rm_driver/JointPos", 300);
    ...
    // 定义一个服务器
    Server server(nh, "right_arm_controller/follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
    // 服务器开始运行
    server.start();
    ros::spin();
