...
{
    ros::init(argc, argv, "left_rm_arm_control");
    ros::NodeHandle nh;
    ...
    get_ArmError = nh.subscribe("/left_rm_driver/ArmError", 100, getArmError_Callback);
    system_error_clean_pub = nh.advertise<std_msgs::Empty>("/left_rm_driver/Clear_System_Err", 300);
    target_pub = nh.advertise<rm_msgs::JointPos>("/left_rm_driver/JointPos", 300);
    ...
    // 定义一个服务器
    Server server(nh, "left_arm_controller/follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
    // 服务器开始运行
    server.start();
    ros::spin();
